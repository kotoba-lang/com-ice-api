(ns ice_api.main
  "Kotodama WASM entrypoint for the ice_api clean-room actor (L4) — Clojure port.
  ...
  facts (`ice_api.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "ice_api")
(def tier "L4")
...
(def entity-specs
  [{:entity "Instrument" :plural "instruments" :id-prefix "iceapi_ins"
   ...
   {:entity "Position"   :plural "positions"   :id-prefix "iceapi_pos"
...
(defn emit-facts
  "EAVT facts for one record: {\"ice_api.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "ice_api-compat" :tier tier :entities entities} 200])
...
(defn start! [] :ice_api-compat/ready)
